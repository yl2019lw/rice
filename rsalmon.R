...
ds <- "msu" #for other data source, change ds value.
countdir <- file.path(project, "out", ds, "salmon/quant") #location of my htseq counts file
outdir <- file.path(project, "out", ds, "deseq2/salmon") #location of output of result
sffiles <- file.path(countdir, list.files(countdir), "quant.sf")

#construct sample info
#sampleFiles <- grep("*.sf", list.files(countdir), value = TRUE)
#sampleNames <- str_replace(sampleFiles, ".count", "") #(HP15_0d_rep1)
sampleNames <- list.files(countdir) #(HP15_0d_rep1)
...
#Please fix name not match bug
#gff3file <- file.path(project, "ref/ensemble", "Oryza_sativa.IRGSP-1.0.37.gff3")
#txdb <- makeTxDbFromGFF(gff3file, format = "gff3", dataSource = "ensemble", organism = "Oryza sativa")
#k <- keys(txdb, keytype = "GENEID")
#df <- select(txdb, keys = k, keytype = "GENEID", columns = "TXNAME")
#tx2gene <- df[, 2:1]  # tx ID, then gene ID

tx2gene <- read.csv(file.path(outdir, "tx2gene.csv"))
txi <- tximport(sffiles, type = "salmon", tx2gene = tx2gene)

ddstxi <- DESeqDataSetFromTximport(txi, colData = sampleTable, design = ~ sampleGroup)
